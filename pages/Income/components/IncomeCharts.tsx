...
export const IncomeCharts: React.FC<IncomeChartsProps> = ({ incomes }) => {
  ...
  const topIncomes = useMemo(() => {
      ...
      // Replaced [...incomes].sort().slice(0, 5) with a single-pass O(N) loop.
      // Expected impact: Faster Dashboard rendering on high-volume income data by reducing O(N log N) sort and full array cloning to O(N).
      const top: Income[] = [];
      for (const inc of incomes) {
          if (top.length < 5) {
              top.push(inc);
              top.sort((a, b) => b.amount - a.amount);
          } else if (inc.amount > top[4].amount) {
              top[4] = inc;
              top.sort((a, b) => b.amount - a.amount);
          }
      }
