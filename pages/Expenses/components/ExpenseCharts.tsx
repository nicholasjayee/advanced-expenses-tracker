...
export const ExpenseCharts: React.FC<ExpenseChartsProps> = ({ expenses }) => {
  ...
  const topExpenses = useMemo(() => {
      ...
      // Replaced [...expenses].sort().slice(0, 5) with a single O(N) pass to find the top 5 expenses.
      // Expected Impact: Reduces time complexity from O(N log N) to O(N) and avoids full array cloning,
      // speeding up rendering of the Expenses Chart tab for large datasets.
      const top = [];
      for (const e of expenses) {
        if (top.length < 5) {
          top.push(e);
          top.sort((a, b) => b.amount - a.amount);
        } else if (e.amount > top[4].amount) {
          top[4] = e;
          top.sort((a, b) => b.amount - a.amount);
        }
      }
