...
export const ExpenseCharts: React.FC<ExpenseChartsProps> = ({ expenses }) => {
  ...
  const topExpenses = useMemo(() => {
      ...
      // Replaced O(N log N) full array sort and slice with an O(N) single-pass loop
      // Expected impact: Faster rendering of the charts component for high-volume expense data
      // by maintaining a local array of top 5 items instead of sorting the entire array.
      const top: typeof expenses = [];
      for (const expense of expenses) {
          if (top.length < 5) {
              top.push(expense);
              top.sort((a, b) => b.amount - a.amount);
          } else if (expense.amount > top[4].amount) {
              top[4] = expense;
              top.sort((a, b) => b.amount - a.amount);
          }
      }
